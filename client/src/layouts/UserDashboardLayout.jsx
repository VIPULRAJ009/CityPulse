...
import { LayoutDashboard, Calendar, LogOut, Home, Bell, X, Check, Search, User, Ticket } from 'lucide-react';
...
const UserDashboardLayout = () => {
  ...
  // Reusing notification logic for now, though it might need specific user endpoints if different
  ...
  const navItems = [
    { name: 'Find events', path: '/', icon: <Search size={20} /> }, // Public Home
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col">
        <div className="p-6 border-b border-slate-700">
          <Link to="/" className="flex items-center space-x-2">
            <h1 className="text-2xl font-bold tracking-tight text-white">CityPulse <span className="text-sm font-normal text-slate-400">User</span></h1>
          </Link>
        </div>

        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => {
            // Check active state. Special case for Home to avoid matching everything
